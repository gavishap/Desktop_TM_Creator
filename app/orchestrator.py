...
Replaces the Sheets daemon. On ``start()`` it spins up its own event loop in a thread,
processes every runnable account (respecting ``max_concurrent``), and handles the
block/backoff loop: on a TM bot-block it closes Chrome (wiping cookies), waits an
escalating cooldown, and retries with a fresh session. Progress, logs, and code prompts
flow to the UI through the shared EventBus / CodeBroker.

Two things are treated separately from ordinary failures:
  * a **bot block** is about this browser session — wipe cookies, short cooldown, retry;
  * a **throttle** is Ticketmaster rate-limiting the whole internet connection — the row
    is put back in the queue for half an hour or more, and after enough throttles in a row
    the app stands down completely rather than hammering an IP that is already unhappy.
"""
...
class Runner:
    ...
    def _due_now(self) -> list[Account]:
        """Accounts ready to start right now — throttled rows stay out until their time."""
    ...
    async def _main(self):
        ...
        try:
            while accounts:
                ...
                # Rows parked by a throttle come back on their own — no clicking required.
                due_at = self.store.next_retry_at(self._account_ids)
                if not due_at:
                    break
                wait = max(1.0, due_at - time.time())
                self.bus.log(f"Waiting {int(wait // 60) + 1}m for throttled account(s) "
                             f"to come back around.", "warn")
    ...
    async def _process_account(self, account: Account):
        ...
        retries = self.config.block.max_retries
        cooldowns = self.config.block.cooldowns or [120]
        ...
        for attempt in range(retries + 1):
            ...
            account.status = IN_PROGRESS
            self.store.save_account(account)
            log(f"Attempt {attempt + 1}/{retries + 1} — launching Chrome")
            ...
            if attempt < retries:
                ...
                account.status = FAILED
                self.store.save_account(account)
                log(f"Giving up after {attempt + 1} attempt(s): {account.error}", "error")
                return
